...
class EmailService {
    ...
    async fetchEmails(accountEmail, folder = "INBOX", limit = 50) {
        console.log();
        ...
        try {
            ...
            try {
                ...
                console.log(`📧 Mailbox exists: ${mailbox ? mailbox.exists : 0} messages`);
                ...
                for await (const message of imap.fetch(range, {
                    ...
                    try {
                        ...
                        const email = {
                            ...
                            isDeleted: folder.toLowerCase().includes("trash") ||
    ...
    async searchEmails(accountEmail, criteria, folder = "INBOX") {
        const client = this.clients.get(accountEmail);
        if (!client)
            throw new Error("Account (searchEmails) not connected");
        const { imap } = client;
        const emails = [];
        const lock = await imap.getMailboxLock(folder);
        try {
    ...
    async sendEmail(accountEmail, to, subject, body, html) {
        const client = this.clients.get(accountEmail);
        if (!client)
            throw new Error("Account (sendEmail) not connected");
        const { smtp } = client;
    ...
    async disconnectAccount(accountEmail) {
        const client = this.clients.get(accountEmail);
        if (client) {
    ...
    async listFolders(accountEmail) {
        const client = this.clients.get(accountEmail);
        if (!client)
            throw new Error("Account (listFolders) not connected");
        const { imap } = client;
...
// IPC Handlers
electron_1.ipcMain.handle("email:connect", async (event, account) => {
...
electron_1.ipcMain.handle("email:fetch", async (event, accountEmail, folder, limit) => {
    try {
...
electron_1.ipcMain.handle("email:search", async (event, accountEmail, criteria, folder) => {
    try {
        const emails = await emailService.searchEmails(accountEmail, criteria, folder);
        return { success: true, emails };
    }
    catch (error) {
        return { success: false, error: error.message };
...
electron_1.ipcMain.handle("email:send", async (event, accountEmail, to, subject, body, html) => {
    try {
        const result = await emailService.sendEmail(accountEmail, to, subject, body, html);
...
electron_1.ipcMain.handle("email:disconnect", async (event, accountEmail) => {
    try {
...
electron_1.ipcMain.handle("email:folders", async (event, accountEmail) => {
    try {
